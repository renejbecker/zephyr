...
static int bridge_offload_addif(struct net_if *br, struct net_if *iface)
{
	...
	if (api->bridge_addif == NULL) {
		return -ENOTSUP;
	}

	return api->bridge_addif(dev, br, iface);
}
...
static int bridge_offload_delif(struct net_if *br, struct net_if *iface)
{
	...
	if (api->bridge_delif == NULL) {
		return -ENOTSUP;
	}

	return api->bridge_delif(dev, br, iface);
}
...
static int bridge_offload_set_status(struct net_if *br, struct net_if *iface, bool enable)
{
	...
	if (enable) {
		if (api->bridge_start == NULL) {
			return -ENOTSUP;
		}

		return api->bridge_start(dev, br, iface);
	}

	if (api->bridge_stop == NULL) {
		return -ENOTSUP;
	}

	return api->bridge_stop(dev, br, iface);
}
...
int bridge_iface_send(struct net_if *iface, struct net_pkt *pkt)
{
	 ...
	 */
	if (!net_pkt_is_l2_bridged(pkt) && net_pkt_family(pkt) == AF_INET &&
		...
		ret = net_arp_prepare(pkt, (struct in_addr *)NET_IPV4_HDR(pkt)->dst,
				      NULL, &arp_pkt);
		switch (ret) {
