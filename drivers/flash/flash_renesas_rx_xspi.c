...
#define DT_DRV_COMPAT renesas_rx_xspi_nor
...
LOG_MODULE_REGISTER(flash_renesas_rx_xspi, CONFIG_FLASH_LOG_LEVEL);
...
#define QPI_SPI_NOR_CMD_SE_4B 0x21
#define QPI_SPI_NOR_CMD_BE_4B 0xDC
#define QPI_SPI_NOR_CMD_CE    0x60

#define SPI_NOR_CMD_QPI_READ_FAST_4B 0xEC
#define SPI_NOR_CMD_QPI_PPQ_4B       0x12

#define SPI_NOR_CMD_QPI_ENTER 0x35 /* QPIEN */
#define SPI_NOR_CMD_QPI_EXIT  0xF5 /* QPIDI */
...
struct flash_renesas_rx_xspi_controller_data {
	xspi_controller_instance_ctrl_t xspi_controller_ctrl;
	struct k_sem sem;
};

struct flash_renesas_rx_xspi_controller_config {
	...
	xspi_controller_timing_setting_t xspi_controller_timing_setting;
...
static spi_flash_erase_command_t qpi_erase_command_set[] = {
	{.command = QPI_SPI_NOR_CMD_SE_4B, .size = SPI_NOR_SECTOR_SIZE},
	{.command = QPI_SPI_NOR_CMD_BE_4B, .size = SPI_NOR_BLOCK_SIZE},
	{.command = QPI_SPI_NOR_CMD_CE, .size = SPI_FLASH_ERASE_SIZE_CHIP_ERASE}};
...
static xspi_controller_table_t qpi_erase_command_table = {
	.p_table = qpi_erase_command_set,
	.length = ARRAY_SIZE(qpi_erase_command_set),
};

static xspi_controller_table_t erase_command_table = {
	.p_table = erase_command_set,
	.length = ARRAY_SIZE(erase_command_set),
};

static xspi_controller_table_t high_speed_erase_command_table = {
	.p_table = high_speed_erase_command_set,
	.length = ARRAY_SIZE(high_speed_erase_command_set),
};

/* Master table that contains all supported command for XSPI
 */
static xspi_controller_command_set_t xspi_command_set[] = {
	/* js25lp256d QPI entry */
	{
		.protocol = SPI_FLASH_PROTOCOL_4S_4S_4S,
		.frame_format = XSPI_CONTROLLER_FRAME_FORMAT_STANDARD,
		.latency_mode = XSPI_CONTROLLER_LATENCY_MODE_FIXED,
		.command_bytes = XSPI_CONTROLLER_COMMAND_BYTES_1,
		.address_bytes = SPI_FLASH_ADDRESS_BYTES_4,

		.read_command = SPI_NOR_CMD_QPI_READ_FAST_4B,
		.program_command = SPI_NOR_CMD_QPI_PPQ_4B,
		...
		.p_erase_commands = &qpi_erase_command_table,
	},

	/* generic SPI entry (1S_1S_1S) */
	{
	...
	/* MX25LW51245G high-speed / OCTAL entry (8D_8D_8D) */
	{
		.protocol = SPI_FLASH_PROTOCOL_8D_8D_8D,
		.frame_format = XSPI_CONTROLLER_FRAME_FORMAT_XSPI_PROFILE_1,
		.latency_mode = XSPI_CONTROLLER_LATENCY_MODE_FIXED,
		.command_bytes = XSPI_CONTROLLER_COMMAND_BYTES_2,
		.address_bytes = SPI_FLASH_ADDRESS_BYTES_4,
...
static xspi_controller_table_t xspi_command_table = {
	.p_table = xspi_command_set,
	.length = ARRAY_SIZE(xspi_command_set),
};

static void acquire_device(const struct device *dev)
{
	struct flash_renesas_rx_xspi_controller_data *dev_data = dev->data;
...
static int
flash_renesas_rx_xspi_controller_write_enable(xspi_controller_instance_ctrl_t *p_instance_ctrl)
{
	...
	err = R_XSPI_CONTROLLER_DirectTransfer(p_instance_ctrl, &set_write_enable_command,
	...
	err = R_XSPI_CONTROLLER_DirectTransfer(p_instance_ctrl, &read_status_command,
...
static int
flash_renesas_rx_xspi_controller_set_protocol(xspi_controller_instance_ctrl_t *p_instance_ctrl,
					      uint32_t protocol)
{
	fsp_err_t err;

	if (protocol != XSPI_QUAD_MODE && protocol != XSPI_OCTO_MODE) {
		LOG_ERR("Unsupported protocol");
		return -EINVAL;
	}

	/* Transfer write enable command */
	err = flash_renesas_rx_xspi_controller_write_enable(p_instance_ctrl);
	if (err != 0) {
		LOG_ERR("Write enable failed");
		return -EIO;
	}

	if (protocol == XSPI_QUAD_MODE) {

		spi_flash_direct_transfer_t qpi_enter_cmd = {
			.command = SPI_NOR_CMD_QPI_ENTER,
			.command_length = (uint8_t)FLASH_1_BYTE_COMMAND,
			.address = 0,
			.address_length = (uint8_t)FLASH_NO_ADDRESS,
			.data = 0,
			.data_length = (uint8_t)FLASH_NO_DATA,
			.dummy_cycles = SPI_NOR_DUMMY_NONE,
		};

		spi_flash_direct_transfer_t rdsr_cmd = {
		...
		err = R_XSPI_CONTROLLER_DirectTransfer(p_instance_ctrl, &qpi_enter_cmd,
						       SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);

		if (err != FSP_SUCCESS) {
			LOG_ERR("Failed to enter QPI mode (QPIEN) (%d)", err);
			return -EIO;
		}

		err = R_XSPI_CONTROLLER_SpiProtocolSet(p_instance_ctrl,
						       SPI_FLASH_PROTOCOL_4S_4S_4S);
		if (err != FSP_SUCCESS) {
			LOG_ERR("Failed to set QPI protocol (4S-4S-4S) (%d)", err);
			return -EIO;
		}

		err = R_XSPI_CONTROLLER_DirectTransfer(p_instance_ctrl, &rdsr_cmd,
						       SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
		if (err != FSP_SUCCESS) {
			LOG_ERR("Failed to read status in QPI mode (RDSR) (%d)", err);
			return -EIO;
		}
	}

	if (protocol == XSPI_OCTO_MODE) {
		...
		err = R_XSPI_CONTROLLER_DirectTransfer(p_instance_ctrl, &opi_set_command,
						       SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
		if (err != FSP_SUCCESS) {
		...
		err = R_XSPI_CONTROLLER_DirectTransfer(p_instance_ctrl,
						       &config_reg2_read_command_opi,
						       SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
		if (err != FSP_SUCCESS) {
		...
		if ((uint8_t)(config_reg2_read_command_opi.data) != CR2_INTERFACE_MODE_DTR_OPI) {
			LOG_ERR("Config reg 2 data incorrect: 0x%02x",
				config_reg2_read_command_opi.data);
			return -EIO;
		}
	}
...
static int flash_renesas_rx_xspi_controller_wait_operation(xspi_controller_instance_ctrl_t *p_ctrl,
							   uint32_t timeout_ms)
{
	...
	while (status.write_in_progress && max_loop_count > 0) {
		k_usleep(50);
		/* Get device status */
		R_XSPI_CONTROLLER_StatusGet(p_ctrl, &status);
		max_loop_count--;
	}
...
static int flash_renesas_rx_xspi_controller_setup_calibrate_data(
	xspi_controller_instance_ctrl_t *p_instance_ctrl)
{
	...
	const xspi_controller_extended_cfg_t *const p_cfg_extend =
		(xspi_controller_extended_cfg_t *)(p_instance_ctrl->p_cfg->p_extend);
...
static inline bool flash_renesas_rx_xspi_controller_is_valid_address(const struct device *dev,
...
static int flash_renesas_rx_xspi_controller_erase(const struct device *dev, off_t offset,
						  size_t len)
{
	...
	if (!flash_renesas_rx_xspi_controller_is_valid_address(dev, offset, len)) {
	...
	while (len > 0) {
		...
		err = flash_renesas_rx_xspi_controller_wait_operation(
			&xspi_controller_data->xspi_controller_ctrl, erase_timeout);
		if (err != 0) {
...
static int flash_renesas_rx_xspi_controller_write(const struct device *dev, off_t offset,
						  const void *data, size_t len)
{
	...
	while (len > 0) {
		...
		err = R_XSPI_CONTROLLER_Write(&xspi_controller_data->xspi_controller_ctrl, p_src,
		...
		err = flash_renesas_rx_xspi_controller_wait_operation(
			&xspi_controller_data->xspi_controller_ctrl, PAGE_PROGRAM_MAX_TIMEOUT_MS);
		if (err != 0) {
...
flash_renesas_rx_xspi_controller_get_parameters(const struct device *dev)
{
	const struct flash_renesas_rx_xspi_controller_config *config = dev->config;

	return &config->xspi_controller_param;
}

static int flash_renesas_rx_xspi_controller_get_size(const struct device *dev, uint64_t *size)
{
	const struct flash_renesas_rx_xspi_controller_config *config = dev->config;
...
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
void flash_renesas_rx_xspi_controller_page_layout(const struct device *dev,
...
static int flash_renesas_rx_xspi_controller_sfdp_read(const struct device *dev, off_t offset,
						      void *data, size_t len)
{
	...
	while (len > 0) {
		...
		err = R_XSPI_CONTROLLER_DirectTransfer(&xspi_controller_data->xspi_controller_ctrl,
						       &sfdp_read_command,
						       SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
		if (err != FSP_SUCCESS) {
...
static int flash_renesas_rx_xspi_controller_read_jedec_id(const struct device *dev, uint8_t *id)
{
	struct flash_renesas_rx_xspi_controller_data *xspi_controller_data = dev->data;
	const struct flash_renesas_rx_xspi_controller_config *config = dev->config;
...
static int flash_renesas_rx_xspi_controller_ex_op(const struct device *dev, uint16_t code,
						  const uintptr_t in, void *out)
{
	...
	err = R_XSPI_CONTROLLER_DirectTransfer(&xspi_controller_data->xspi_controller_ctrl,
					       &command, SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
	if (err != FSP_SUCCESS) {
...
static DEVICE_API(flash, flash_renesas_rx_xspi_controller_api) = {
	.erase = flash_renesas_rx_xspi_controller_erase,
	.write = flash_renesas_rx_xspi_controller_write,
	.read = flash_renesas_rx_xspi_controller_read,
	.get_parameters = flash_renesas_rx_xspi_controller_get_parameters,
	.get_size = flash_renesas_rx_xspi_controller_get_size,
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
	.page_layout = flash_renesas_rx_xspi_controller_page_layout,
#endif /* CONFIG_FLASH_PAGE_LAYOUT */
#if defined(CONFIG_FLASH_JESD216_API)
	.sfdp_read = flash_renesas_rx_xspi_controller_sfdp_read,
	.read_jedec_id = flash_renesas_rx_xspi_controller_read_jedec_id,
#endif /* CONFIG_FLASH_JESD216_API */
#if defined(CONFIG_FLASH_EX_OP_ENABLED)
	.ex_op = flash_renesas_rx_xspi_controller_ex_op,
#endif /* CONFIG_FLASH_EX_OP_ENABLED */
};

static int flash_renesas_rx_xspi_controller_init(const struct device *dev)
{
	const struct flash_renesas_rx_xspi_controller_config *config = dev->config;
	struct flash_renesas_rx_xspi_controller_data *xspi_controller_data = dev->data;
	uint32_t clock_freq;
	...
	if (config->data_mode == XSPI_DUAL_MODE) {
		LOG_ERR("XSPI DUAL mode not supported");
		return -ENOTSUP;
	...
	k_sem_init(&xspi_controller_data->sem, 1, 1);

	/** Initialize the XSPI controller module */
	err = R_XSPI_CONTROLLER_Open(&xspi_controller_data->xspi_controller_ctrl,
	...
	err = flash_renesas_rx_xspi_controller_set_protocol(
		&xspi_controller_data->xspi_controller_ctrl, config->data_mode);
	if (err != 0) {
		LOG_ERR("Failed to set protocol to mode %d", config->data_mode);
		return -EIO;
	}
