...
LOG_MODULE_REGISTER(pwm_renesas_rx_gpt_pwm, CONFIG_PWM_LOG_LEVEL);

#define DT_DRV_COMPAT renesas_rx_gpt_pwm
...
static uint32_t pwm_renesas_rx_gtior_calculate(gpt_pin_level_t const stop_level)
{
...
static int pwm_renesas_rx_apply_gtior_config(gpt_instance_ctrl_t *const p_ctrl,
	...
	if (p_extend->gtior_setting.gtior == 0) {
		...
		if (p_extend->gtioca.output_enabled) {
			uint32_t gtioca_gtior =
				pwm_renesas_rx_gtior_calculate(p_extend->gtioca.stop_level);
		...
		if (p_extend->gtiocb.output_enabled) {
			uint32_t gtiocb_gtior =
				pwm_renesas_rx_gtior_calculate(p_extend->gtiocb.stop_level);
...
static int pwm_renesas_rx_set_cycles(const struct device *dev, uint32_t pin, uint32_t period_cycles,
				     uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	pwm_renesas_rx_apply_gtior_config(&data->fsp_ctrl, &data->fsp_cfg);
...
static int pwm_renesas_rx_get_cycles_per_sec(const struct device *dev, uint32_t pin,
					     uint64_t *cycles)
{
	struct pwm_renesas_rx_data *data = dev->data;
	timer_info_t info;
...
static int pwm_renesas_rx_configure_capture(const struct device *dev, uint32_t pin,
...
static int pwm_renesas_rx_enable_capture(const struct device *dev, uint32_t pin)
{
	struct pwm_renesas_rx_data *data = dev->data;
	fsp_err_t err;
	...
	icu_connect_event(data->fsp_cfg.cycle_end_irq, data->overflow_event);
	icu_connect_event(data->extend_cfg.capture_a_irq, data->capture_a_event);

	return 0;
}

static int pwm_renesas_rx_disable_capture(const struct device *dev, uint32_t pin)
{
	struct pwm_renesas_rx_data *data = dev->data;
	fsp_err_t err;
	...
	icu_connect_event(data->fsp_cfg.cycle_end_irq, ELC_EVENT_NONE);
	icu_connect_event(data->extend_cfg.capture_a_irq, ELC_EVENT_NONE);
	...
	if (p_args->event == TIMER_EVENT_CAPTURE_A) {
		if (p_args->capture != 0U) {
			...
			if (data->capture.continuous == false) {
				pwm_renesas_rx_disable_capture(dev, GPT_IO_PIN_GTIOCA);
			}
		}
...
{
	struct pwm_renesas_rx_data *data = dev->data;
	const struct pwm_renesas_rx_config *cfg = dev->config;
	int err;
...
#ifdef CONFIG_PWM_CAPTURE
#define PWM_RX_IRQ_CONFIG_INIT(index)                                                              \
	do {                                                                                       \
		int gtioca_irq = DT_IRQ_BY_NAME(DT_INST_PARENT(index), gtioca, irq);               \
		int overflow_irq = DT_IRQ_BY_NAME(DT_INST_PARENT(index), overflow, irq);           \
                                                                                                   \
		if (128 <= gtioca_irq && gtioca_irq < 144) {                                       \
			R_ICU->SLIXR[gtioca_irq - 128].SLIXR = EVENT_GPT_CAPTURE_COMPARE_A(        \
				DT_PROP(DT_INST_PARENT(index), channel));                          \
		} else if (144 <= gtioca_irq) {                                                    \
			R_ICU->SLIR[gtioca_irq - 144].SLIR = EVENT_GPT_CAPTURE_COMPARE_A(          \
				DT_PROP(DT_INST_PARENT(index), channel));                          \
		}                                                                                  \
                                                                                                   \
		if (128 <= overflow_irq && overflow_irq < 144) {                                   \
			R_ICU->SLIXR[overflow_irq - 128].SLIXR = EVENT_GPT_COUNTER_OVERFLOW(       \
				DT_PROP(DT_INST_PARENT(index), channel));                          \
		} else if (144 <= overflow_irq) {                                                  \
			R_ICU->SLIR[overflow_irq - 144].SLIR = EVENT_GPT_COUNTER_OVERFLOW(         \
				DT_PROP(DT_INST_PARENT(index), channel));                          \
		}                                                                                  \
                                                                                                   \
		IRQ_CONNECT(DT_IRQ_BY_NAME(DT_INST_PARENT(index), gtioca, irq),                    \
			    DT_IRQ_BY_NAME(DT_INST_PARENT(index), gtioca, priority),               \
			    pwm_rx_gpt_ccmpa_isr, DEVICE_DT_INST_GET(index), 0);                   \
		IRQ_CONNECT(DT_IRQ_BY_NAME(DT_INST_PARENT(index), overflow, irq),                  \
			    DT_IRQ_BY_NAME(DT_INST_PARENT(index), overflow, priority),             \
			    pwm_rx_gpt_ovf_isr, DEVICE_DT_INST_GET(index), 0);                     \
	} while (0)
...
#define PWM_RX_GPT_INIT(index)                                                                        \
	...
	static const gpt_extended_cfg_t g_timer1_extend_##index = {                                \
		...
		.capture_a_ipl = DT_IRQ_BY_NAME(DT_INST_PARENT(index), gtioca, priority),          \
		.capture_b_ipl = BSP_IRQ_DISABLED,                                                 \
		.capture_a_irq = DT_IRQ_BY_NAME(DT_INST_PARENT(index), gtioca, irq),               \
	...
	static struct pwm_renesas_rx_data pwm_renesas_rx_data_##index = {                          \
		.fsp_cfg =                                                                         \
			{                                                                          \
				.mode = TIMER_MODE_PWM,                                            \
				.source_div = DT_ENUM_IDX(DT_INST_PARENT(index), divider),         \
				.channel = DT_PROP(DT_INST_PARENT(index), channel),                \
				.cycle_end_ipl =                                                   \
					DT_IRQ_BY_NAME(DT_INST_PARENT(index), overflow, priority), \
				.cycle_end_irq =                                                   \
					DT_IRQ_BY_NAME(DT_INST_PARENT(index), overflow, irq),      \
			},                                                                         \
		.extend_cfg = g_timer1_extend_##index,                                             \
		.capture_a_event =                                                                 \
			EVENT_GPT_CAPTURE_COMPARE_A(DT_PROP(DT_INST_PARENT(index), channel)),      \
		.overflow_event =                                                                  \
			EVENT_GPT_COUNTER_OVERFLOW(DT_PROP(DT_INST_PARENT(index), channel)),       \
	};                                                                                         \
	static const struct pwm_renesas_rx_config pwm_renesas_rx_config_##index = {                \
	...
	DEVICE_DT_INST_DEFINE(index, pwm_renesas_rx_init_##index, NULL,                            \
			      &pwm_renesas_rx_data_##index, &pwm_renesas_rx_config_##index,        \
			      POST_KERNEL, CONFIG_PWM_INIT_PRIORITY, &pwm_renesas_rx_driver_api);

DT_INST_FOREACH_STATUS_OKAY(PWM_RX_GPT_INIT);
