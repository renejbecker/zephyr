...
int renesas_ra_eswm_bridge_addif(const struct device *eswm_dev, uint8_t ch,
				  struct net_if *br)
{
...
int renesas_ra_eswm_bridge_delif(const struct device *eswm_dev, uint8_t ch,
				  struct net_if *br)
{
	struct eswm_renesas_ra_data *eswm = eswm_dev->data;
...
int renesas_ra_eswm_bridge_start(const struct device *eswm_dev, uint8_t ch,
				  struct net_if *br)
{
	struct eswm_renesas_ra_data *eswm = eswm_dev->data;
	int ret;
	 ...
	 * ports are recorded in bridge_port_mask, not in bridge_addif. */
	...
	rmac_bridge_set_flood_mask(
		ch, (eswm->bridge_port_mask | LAYER3_SWITCH_PORT_BITMASK_PORT2) & ~BIT(ch));

	return 0;
}

int renesas_ra_eswm_bridge_stop(const struct device *eswm_dev, uint8_t ch,
				 struct net_if *br)
{
	struct eswm_renesas_ra_data *eswm = eswm_dev->data;
