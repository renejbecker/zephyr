...
static int eth_renesas_ra_bridge_addif(const struct device *dev, struct net_if *br,
				       struct net_if *iface)
{
	const struct eth_renesas_ra_config *config = dev->config;
	struct eth_renesas_ra_data *data = dev->data;

	return renesas_ra_eswm_bridge_addif(config->eswm_dev, data->fsp_cfg.channel, br);
}

static int eth_renesas_ra_bridge_delif(const struct device *dev, struct net_if *br,
				       struct net_if *iface)
{
	const struct eth_renesas_ra_config *config = dev->config;
	struct eth_renesas_ra_data *data = dev->data;

	return renesas_ra_eswm_bridge_delif(config->eswm_dev, data->fsp_cfg.channel, br);
}

static int eth_renesas_ra_bridge_start(const struct device *dev, struct net_if *br,
				       struct net_if *iface)
{
	const struct eth_renesas_ra_config *config = dev->config;
	struct eth_renesas_ra_data *data = dev->data;

	return renesas_ra_eswm_bridge_start(config->eswm_dev, data->fsp_cfg.channel, br);
}

static int eth_renesas_ra_bridge_stop(const struct device *dev, struct net_if *br,
				      struct net_if *iface)
{
	const struct eth_renesas_ra_config *config = dev->config;
	struct eth_renesas_ra_data *data = dev->data;

	return renesas_ra_eswm_bridge_stop(config->eswm_dev, data->fsp_cfg.channel, br);
}
...
#if defined(CONFIG_ETH_RENESAS_RA_HW_BRIDGE)
	.bridge_addif = eth_renesas_ra_bridge_addif,
	.bridge_delif = eth_renesas_ra_bridge_delif,
	.bridge_start = eth_renesas_ra_bridge_start,
	.bridge_stop = eth_renesas_ra_bridge_stop,
