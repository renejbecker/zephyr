...
LOG_MODULE_REGISTER(eth_renesas_rx, CONFIG_ETHERNET_LOG_LEVEL);
...
struct eswm_renesas_rx_config {
	...
	struct clock_control_rx_subsys_cfg pclk_subsys;
	struct clock_control_rx_subsys_cfg ethphyclk_subsys;
...
#if defined(CONFIG_ETH_RENESAS_RX_USE_ZERO_COPY)
struct eth_renesas_rx_buf_header {
	uint8_t *buf;
};
#endif

struct eth_renesas_rx_data {
	struct net_if *iface;
	uint8_t mac_addr[NET_ETH_ADDR_LEN];

#if defined(CONFIG_ETH_RENESAS_RX_USE_ZERO_COPY)
	/* DMA buffer header */
	struct eth_renesas_rx_buf_header *tx_buf_header;
	uint8_t tx_buf_idx;
	uint8_t tx_buf_num;
#else
	/* staging buffer */
	...
	K_KERNEL_STACK_MEMBER(rx_thread_stack, CONFIG_ETH_RENESAS_RX_RX_THREAD_STACK_SIZE);
	struct k_sem rx_sem;
#if defined(CONFIG_ETH_RENESAS_RX_USE_HW_WRITEBACK)
	struct k_sem tx_sem;
#endif
	uint8_t phy_link_up;
...
static void phy_link_cb(const struct device *phy_dev, struct phy_link_state *state, void *eth_dev)
{
	...
	if (state->is_up) {
		...
		r_rmac_phy_set_operation_mode(data->fsp_cfg->channel, RENESAS_RX_ETHA_DISABLE_MODE);
		r_rmac_phy_set_operation_mode(data->fsp_cfg->channel, RENESAS_RX_ETHA_CONFIG_MODE);

		switch (state->speed) {
		case LINK_HALF_10BASE:
			__fallthrough;
		case LINK_FULL_10BASE:
			data->fsp_ctrl->p_reg_rmac->MPIC_b.LSC = RENESAS_RX_MPIC_LSC_10;
			break;
		case LINK_HALF_100BASE:
			__fallthrough;
		case LINK_FULL_100BASE:
			data->fsp_ctrl->p_reg_rmac->MPIC_b.LSC = RENESAS_RX_MPIC_LSC_100;
			break;
		case LINK_HALF_1000BASE:
			__fallthrough;
		case LINK_FULL_1000BASE:
			data->fsp_ctrl->p_reg_rmac->MPIC_b.LSC = RENESAS_RX_MPIC_LSC_1000;
			break;
		default:
		...
		r_rmac_phy_set_operation_mode(data->fsp_cfg->channel,
					      RENESAS_RX_ETHA_OPERATION_MODE);
...
static void eth_rmac_cb(ether_callback_args_t *args)
{
	...
	struct eth_renesas_rx_data *data = eth_dev->data;
...
static void eth_renesas_rx_init_iface(struct net_if *iface)
{
...
#if defined(CONFIG_NET_STATISTICS_ETHERNET)
static struct net_stats_eth *eth_renesas_rx_get_stats(const struct device *dev)
{
	struct eth_renesas_rx_data *data = dev->data;
...
const struct device *eth_renesas_rx_get_phy(const struct device *dev)
{
	const struct eth_renesas_rx_config *config = dev->config;
...
static int eth_renesas_rx_send(const struct device *dev, struct net_pkt *pkt)
{
	struct eth_renesas_rx_data *data = dev->data;
...
static void renesas_rx_eth_receive(const struct device *dev)
{
	struct eth_renesas_rx_data *data = dev->data;
...
static void eth_rx_thread(void *arg1, void *unused1, void *unused2)
{
	...
	while (1) {
		...
		renesas_rx_eth_receive(dev);
	}
}

static int renesas_rx_eswm_init(const struct device *dev)
{
	struct eswm_renesas_rx_data *data = dev->data;
	const struct eswm_renesas_rx_config *config = dev->config;
	...
	rx_grp_intc_callback_set(config->gwdi_ctrl, config->gwdi_num,
						 layer3_switch_gwdi_isr,
						 (void *)data->fsp_ctrl);

	/* Enable Group interrupt.*/
	rx_grp_intc_enable(config->gwdi_ctrl, config->gwdi_num);
...
static int renesas_rx_eth_init(const struct device *dev)
{
	...
	k_thread_create(&data->rx_thread, data->rx_thread_stack,
			...
			NULL, NULL, K_PRIO_COOP(CONFIG_ETH_RENESAS_RX_RX_THREAD_PRIORITY), 0,
			K_NO_WAIT);
	k_thread_name_set(&data->rx_thread, "eth_renesas_rx_receive_thread");

	return 0;
}

static const struct ethernet_api eth_renesas_rx_api = {
	.iface_api.init = eth_renesas_rx_init_iface,
#if defined(CONFIG_NET_STATISTICS_ETHERNET)
	.get_stats = eth_renesas_rx_get_stats,
#endif /* CONFIG_NET_STATISTICS_ETHERNET */
	.get_capabilities = eth_renesas_rx_get_capabilities,
	.get_phy = eth_renesas_rx_get_phy,
	.send = eth_renesas_rx_send,
};

#define DT_DRV_COMPAT renesas_rx_eswm

#define ETH_USE_INTERNAL_PHY_CLK(id)                                                               \
	COND_CODE_1(UTIL_AND(DT_NODE_HAS_COMPAT(id, renesas_rx_rmac),                     \
...
static struct eswm_renesas_rx_data eswm_data = {
	.fsp_ctrl = &eswm_ctrl,
	.fsp_cfg = &eswm_cfg,
};
static struct eswm_renesas_rx_config eswm_config = {
	...
	ETHER_RX_RMAC_GRP_INTC_CONFIG_INIT(0)
};
DEVICE_DT_INST_DEFINE(0, renesas_rx_eswm_init, NULL, &eswm_data, &eswm_config, POST_KERNEL,
		      CONFIG_ESWM_RENESAS_RX_INIT_PRIORITY, NULL);
...
#if defined(CONFIG_ETH_RENESAS_RX_USE_ZERO_COPY)
#define ETH_RENESAS_RX_DATA_BUF_MODE ETHER_ZEROCOPY_ENABLE
#define ETH_TX_BUF_HEADER_DECLARE(idx, n)                                                          \
	{                                                                                          \
		.buf = ETH_TX_BUF_PTR_DECLARE(idx, n),                                             \
	}
#define ETH_RENESAS_RX_DATA_BUF_DECLARE(n)                                                         \
	struct eth_renesas_rx_buf_header eth##n##_tx_buf_header[] = {                              \
		LISTIFY(ETH_TX_BUF_NUM(n), ETH_TX_BUF_HEADER_DECLARE, (,), n),			   \
	}
#define ETH_RENESAS_RX_DATA_BUF_PROP_DECLARE(n)                                                    \
	.tx_buf_header = eth##n##_tx_buf_header, .tx_buf_idx = 0, .tx_buf_num = ETH_TX_BUF_NUM(n)
#else /* CONFIG_ETH_RENESAS_RX_USE_ZERO_COPY */
#define ETH_RENESAS_RX_DATA_BUF_MODE ETHER_ZEROCOPY_DISABLE
#define ETH_RENESAS_RX_DATA_BUF_DECLARE(n)                                                         \
...
#endif /* CONFIG_ETH_RENESAS_RX_USE_ZERO_COPY */
#define ETH_RENESAS_RX_INIT(n)                                                                     \
	...
	ETH_RENESAS_RX_DATA_BUF_DECLARE(n);							   \
                                                                                                   \
	...
	static ether_cfg_t eth##n##_cfg = {                                                        \
		...
		.zerocopy = ETH_RENESAS_RX_DATA_BUF_MODE,					   \
	...
	static struct eth_renesas_rx_data eth##n##_renesas_rx_data = {                             \
		.mac_addr = DT_INST_PROP_OR(n, local_mac_address, {0}),                            \
		.fsp_ctrl = &eth##n##_ctrl,                                                        \
		.fsp_cfg = &eth##n##_cfg,                                                          \
		ETH_RENESAS_RX_DATA_BUF_PROP_DECLARE(n)};					   \
	...
	ETH_NET_DEVICE_DT_INST_DEFINE(n, renesas_rx_eth_init, NULL, &eth##n##_renesas_rx_data,     \
				      &eth##n##_renesas_rx_config, CONFIG_ETH_INIT_PRIORITY,       \
				      &eth_renesas_rx_api, NET_ETH_MTU /*MTU*/);

DT_INST_FOREACH_STATUS_OKAY(ETH_RENESAS_RX_INIT);
